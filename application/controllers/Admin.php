...
class Admin extends CI_Controller
{
    ...
    {
        $getInbox   = $this->M_Global->get_list("contact_us", "status = 'Unread' ")->num_rows();

        echo json_encode($getInbox);
    }
    ## end inbox
    ...
    ## page edit parent
    public function page_edit_menu_setting_parent($category_id)
    {
        $data['title']          = "Milou - CMS Menu Settings";

        $parent                 = $this->uri->segment(2);
        $data['getCategory1']   = $this->M_Global->getmultiparam("category", "category_id = '$category_id' and category_level = '$parent' ")->row_array();
        $categoryUrl            = $data['getCategory1']['category_url'];

        $data['getCategory2']   = $this->M_Global->getmultiparam("category", "parent_id = 0 AND category_level = 'Parent' and category_id != '$category_id' order by category_position asc ")->result_array();
        ...
        $this->load->view('cms/components/navbar', $data);
        $this->load->view('cms/components/sidebar', $data);
        $this->load->view('cms/crud-menu-setting/edit-parent', $data);
        $this->load->view('cms/components/footer', $data);
        $this->load->view('cms/components/modal', $data);
    }

    ## page edit child
    public function page_edit_menu_setting_child($category_id)
    {
        $data['title']          = "Milou - CMS Menu Settings";

        $child                  = $this->uri->segment(2);
        $data['getCategory1']   = $this->M_Global->getmultiparam("category", "category_id = '$category_id' and category_level = 'Child' ")->row_array();
        $categoryUrl            = $data['getCategory1']['category_url'];
        $data['getCategory2']   = $this->M_Global->getmultiparam("category", "parent_id = 0 AND category_level = 'Parent' AND category_id != '$category_id' order by category_position asc")->result_array();
        $parentId               = $data['getCategory1']['parent_id'];
        $data['getParent']      = $this->M_Global->getmultiparam("category", "category_id = '$parentId' ")->row_array();
        ...
        $this->load->view('cms/components/navbar', $data);
        $this->load->view('cms/components/sidebar', $data);
        $this->load->view('cms/crud-menu-setting/edit-child', $data);
        $this->load->view('cms/components/footer', $data);
        $this->load->view('cms/components/modal', $data);
    }

    ## edit parent
    public function edit_menu_setting_parent()
    {
        $categoryId             = $this->input->post("category_id", true);
        $categoryPositionOld    = $this->input->post("category_position", true);
        $categoryLevel          = $this->input->post("category_level", true);
        $categoryPage           = $this->input->post("category_page", true);
        $menuName               = $this->input->post("menu_name", true);
        $status                 = $this->input->post("status", true);
        ...
        $getStaticPage1         = $this->M_Global->getmultiparam("static_page", "static_page_id = '$categoryUrl' ")->row_array();
        $getStaticPage2         = $this->M_Global->getmultiparam("static_page", "category_id = '$categoryId' ")->row_array();
        $staticPageIdOn         = isset($getStaticPage2['static_page_id']) ? $getStaticPage2['static_page_id'] : "";

        $checkCategory          = $this->M_Global->getmultiparam("category", "parent_id = '$categoryId' ")->result_array();
        ...
        if ($categoryLevel == "Child") {
            ...
            $getCategoryParent  = $this->M_Global->get_list("category", "category_id = '$parent' ")->row_array();
            $parentName         = $getCategoryParent['category_name'];

            $getCategoryChild   = $this->M_Global->getmultiparam("category", "parent_id = '$parent' AND category_level = 'Child' ")->result_array();
            $numbersPosition    = array_column($getCategoryChild, 'category_position');

            if (empty($numbersPosition)) {
                $categoryPosition = 1;
            } else {
                $maxNumbers             = max($numbersPosition);
                $categoryPosition       = $maxNumbers + 1;
            ...
            ## update all parent
            $getTargetsParent   = $this->M_Global->getmultiparam("category", "category_position > '$categoryPositionOld' and category_level = 'Parent' order by category_position asc")->result_array();

            if (!empty($getTargetsParent)) {
                foreach ($getTargetsParent as $target) {
                    ...
                    $this->M_Global->update_data("category_id = '$catTargetId' ", $position, "category");
                }
            }

            ## update all category level child to parent
            $getCategoryAllParent   = $this->M_Global->getmultiparam("category", "parent_id = 0 AND category_level = 'Parent' order by category_position asc ")->result_array();
            $numberPositionParent   = array_column($getCategoryAllParent, 'category_position');
            $maxNumberParent        = max($numberPositionParent) + 1;

            if ($checkCategory) {
                foreach ($checkCategory as $category) {
                    $categoryIdChild  = $category["category_id"];
                    ...
                    // echo "<pre>";
                    // print_r($allChild);
                    $this->M_Global->update_data("category_id = '$categoryIdChild' ", $allChild, "category");
                }
            }
        } else {
            $updateData = [
                "category_name"     => $menuName,
                "category_page"     => $categoryPage,
                "category_position" => $categoryPositionOld,
                "category_url"      => $slug,
                "status"            => $status,
                "updated_at"        => date('Y-m-d H:i:s', strtotime('now'))
            ];
        ...
        //die;
        $updatedMain = $this->M_Global->update_data("category_id = '$categoryId' ", $updateData, "category");
        ...
        ## update static page to not available
        $staticPageNew = [
            "category_id"   => $categoryId
        ];
        $updateUseDefault = $this->M_Global->update_data("static_page_id = '$categoryUrl' ", $staticPageNew, "static_page");
        ...
        ## end update static page to not available

        ## update static page to available
        if ($categoryUrl == null || $categoryPage == "Module" || $categoryUrl != $staticPageIdOn) {
            $staticPageOn = [
                "category_id"   => 0,
                "updated_at"    => date('Y-m-d H:i:s', strtotime('now'))
            ];
            $updateDefault = $this->M_Global->update_data("static_page_id = '$staticPageIdOn' ", $staticPageOn, "static_page");
        ...
        ## end update static page to available
    }

    public function edit_menu_setting_child()
    {
        $categoryId             = $this->input->post("category_id", true);
        $parentIdOld            = $this->input->post("parent_id", true);
        $categoryPositionOld    = $this->input->post("category_position", true);
        $categoryLevel          = $this->input->post("category_level", true);
        $categoryPage           = $this->input->post("category_page", true);
        $menuName               = $this->input->post("menu_name", true);
        ...
        if ($categoryLevel == "Parent") {
            ...
            $getCategoryParent  = $this->M_Global->getmultiparam("category", "parent_id = 0 AND category_level = 'Parent' ")->result_array();
            $numbersPosition    = array_column($getCategoryParent, 'category_position');

            $checkCategoryParent    = $this->M_Global->get_list("category", "category_id = '$parentIdOld' ")->row_array();
            $categoryIdParent       = isset($checkCategoryParent['category_id']) ? $checkCategoryParent['category_id'] : "";
            ...
            ## update all child
            $getTargetsChild    = $this->M_Global->getmultiparam("category", "category_position > '$categoryPositionOld' and category_level = 'Child' and parent_id = '$categoryIdParent' order by category_position asc ")->result_array();

            if (!empty($getTargetsChild)) {
                foreach ($getTargetsChild as $target) {
                    $catTargetId        = $target['category_id'];
                    $catPositionTargets = $target['category_position'] - 1;
            ...
            $parent             = $this->input->post("parent", true);
            $checkParentOld     = $this->M_Global->get_list("category", "category_id = '$categoryId' ")->row_array();
            $parentOld          = $checkParentOld['parent_id'];

            if ($parent != $parentOld) {

                $checkCategoryChildTarget   = $this->M_Global->getmultiparam("category", "parent_id = '$parent' and category_level = 'Child' ")->result_array();
                $numberPositionChildTarget  = array_column($checkCategoryChildTarget, 'category_position');

                if (!empty($numberPositionChildTarget)) {
                    $maxNumberChildTarget   = max($numberPositionChildTarget);
                    $categoryPosition       = $maxNumberChildTarget + 1;
                } else {
                    $categoryPosition       = 1;
                }

                ## for update parent name
                $getParentName  = $this->M_Global->getmultiparam("category", "category_id = '$parent' and category_level = 'Parent' ")->row_array();
                $parentName     = $getParentName['category_name'];
                ## end for update parent name
                ...
                ## update category child old
                $checkCategoryChildOld  = $this->M_Global->getmultiparam("category", "parent_id = '$parentOld' and category_level = 'Child' and category_position > '$categoryPositionOld' ")->result_array();

                if ($checkCategoryChildOld) {
                    foreach ($checkCategoryChildOld as $targetChildOld) {
                        $targetIdChild           = $targetChildOld['category_id'];
                        $positionTargetChild     = $targetChildOld['category_position'] - 1;

                        $positionAllChildOld = [
                            "category_position"     => $positionTargetChild,
                            "updated_at"            => date('Y-m-d H:i:s', strtotime('now'))
                        ];
                        $this->M_Global->update_data("category_id = '$targetIdChild' ", $positionAllChildOld, "category");
                    }
                }
                ## end update category child old
            } else {
                $updateData = [
                    "category_name"     => $menuName,
                    "category_page"     => $categoryPage,
    ...
    public function remove_menu_setting()
    {
        ...
        if ($categoryLevel == "Parent" && $getCategory['parent_id'] == 0) {
            ...
            $getTargets         = $this->M_Global->getmultiparam("category", "category_position > '$catPositionOld' and category_level = 'Parent' and parent_id = '$parentId' ")->result_array();
        ...
        } elseif ($getCategory['category_level'] == "Child") {
        ...
        if ($delete) {
            ...
            $this->M_Global->update_data("static_page_id = '$staticPageIdOn' ", $staticPageOn, "static_page");
        ...
        if ($getCategory['category_level'] == "Parent" && $getCategory['parent_id'] == 0) {
            ...
            if ($categoryPosition == 1) {
                $this->session->set_flashdata(
                    ...
                    <div>Sorry, this menu was top position!</div>
                ...
                if ($updateTargets) {
                    ...
                    $this->M_Global->update_data("category_id = '$categoryId' ", $positionOld, "category");
        ...
        if ($getCategory1['category_level'] == "Parent" && $getCategory1['parent_id'] == 0) {
            ...
            if ($categoryPosition == $maxNumbers) {
                $this->session->set_flashdata(
                    ...
                    <div>Sorry, this menu was bottom position!</div>
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>'
                );
